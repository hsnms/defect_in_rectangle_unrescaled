...
void **nrfunc3new( double **f, double **u, int n,double *q, int a,int b,double R0, double K, double H)
{
	       	...
	       	for(i=b+2;i<a-b;i++)//part 2.0
	{
		for (j=2;j<=1+b;j++)
		{
			...
			for(i=a+b+1;i<(int)(n*R0-R0+1);i++)//part 2
	{
		for (j=2;j<=1+b;j++)
		{
			v1=u[i][j+1]-u[i][j];
			 	 ...
			 	 for (i=b+2;i<=a-b-1;i++)// part 4.0
	{
        	...
        	f[i][j]=K*pow((v4+v5)/(2.0*h),2)+K*pow((v1+v2)/(2.0*h),2);
			 ...
			 for (i=a+b+1;i<=(int)(n*R0-R0+1)-1;i++)//part 4
	{
			v1=u[i][2]-u[i][1];
		    v2=u[i][2]-u[i][1];
			v4=u[i+1][1]-u[i][1];
	...
	for (i=a-b+1;i<=a+b-1;i++)//part 10.2
	{
		    v1=u[i][b+2]-u[i][b+1];
		    v2=u[i][b+2]-u[i][b+1];
			v4=u[i+1][b+1]-u[i][b+1];
	...
	for (i=(int)(n*R0-R0+1)-b+1;i<=(int)(n*R0-R0+1)-1;i++)//part 11
	{
			...
			i=1;j=b+1; //part 17
			v1=u[i][j+1]-u[i][j];
			v2=u[i][j+1]-u[i][j]; //v2=u[i][j]-u[i][j-1] is wrong;
			v4=u[i+1][j]-u[i][j];
			v5=u[i+1][j]-u[i][j];
	        	f[i][j]=K*pow((v4+v5)/(2.0*h),2)+K*pow((v1+v2)/(2.0*h),2);
